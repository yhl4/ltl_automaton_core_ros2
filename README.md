...
1. **规划核心与 ROS 2 解耦**  
   TS、Büchi、Product 和搜索算法不依赖 `rclpy`。

2. **保持原有规划语义**  
   不因 ROS 2 迁移随意改变 Product Automaton 或接受运行定义。

3. **ROS 2 节点只负责协调与通信**  
   节点负责参数、Topic、Service、状态同步和消息转换。

4. **重规划从当前实际 TS 状态出发**  
   任务变化或意外状态出现时，使用当前 TS 状态重新计算接受运行。

5. **每个阶段保持可构建、可测试、可回退**  
   功能迁移建议采用独立提交，避免将无关问题混入同一提交。
